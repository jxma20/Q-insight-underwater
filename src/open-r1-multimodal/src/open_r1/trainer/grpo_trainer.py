...
def enforce_flash_attention_2(model):
    """
    遍历模型中的所有注意力层，手动设置启用 Flash Attention 2.0 的标志。
    这是为了绕过 DeepSpeed ZeRO-3 初始化模型在 CPU/Meta 设备上时，
    Flash Attention 2.0 自动禁用的问题。
    """
    ...
    for name, module in model.named_modules():
        # Qwen2.5-VL 可能有两种 FA2 实现: 文本部分和视觉部分
        # Qwen2_5_VLVisionFlashAttention2 是您在 qinsight_multi_task.py 中已经补丁过的模块
        if isinstance(module, Qwen2_5_VLVisionFlashAttention2):
            # 这是一个关键的内部标志位，设置它告诉模块即使在 DeepSpeed 初始化下也要使用 FA2
            if not hasattr(module, "_flash_attn_uses_top_level_vars"):
                # 如果是新的 Qwen 版本，可能需要查看 Qwen2VLAttention 的实现
                print(f"Warning: Module {name} is {module.__class__.__name__}, but missing _flash_attn_uses_top_level_vars.")
                continue

            # ** 核心修复 **：手动设置标志位
            module._flash_attn_uses_top_level_vars = True
            # print(f"Enabled FA2 on: {name}") 
            
            # 此外，如果 Qwen2.5-VL 的文本注意力层也有 Flash Attention 实现，
            # 例如 Qwen2.5Attention，也需要对它进行检查和设置，
            # 但针对 Vision Attention 的修复应该能解决您当前的问题。
...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # enforce_flash_attention_2(model)
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                # enforce_flash_attention_2(self.ref_model)
            elif "Aria" in model_id:
