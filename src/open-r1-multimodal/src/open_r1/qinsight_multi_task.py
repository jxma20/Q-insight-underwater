...
# DIST_QUESTION_PROMPT = 'Analyze the given image and determine if it contains any of the following distortions: "color cast", "contrast", "blur or noise", or "exposure". If a distortion is present, classify its class: "color cast": "Blue/Green"; "contrast": "Too high/Too low", "blur or noise": "Blur exists/Noise exists/Both blur and noise", "exposure": "Overexposed/Underexposed". Return the result in JSON format with the following keys: "distortion_class": The detected distortion (or "null" if none). and "severity": The severity level (or "null" if none).'
DIST_QUESTION_PROMPT = """Analyze the visual quality of the image. If any distortion is present, identify its category and specific manifestation based on the following rules:

- If "color cast": set "severity" to "Blue" or "Green".
- If "contrast": set "severity" to "Too high" or "Too low".
- If "blur or noise": set "severity" to "Blur exists", "Noise exists", or "Both blur and noise".
- If "exposure": set "severity" to "Overexposed" or "Underexposed".

Return the result in JSON format within <answer> tags with these exact keys:
"distortion_class": The category name (e.g., "color cast").
"severity": The specific manifestation from the rules above(e.g., "Green").
If no distortion is found, set both to "null"."""
    ...
    def __getitem__(self, index):
        ...
        if self.score_samples and self.dist_samples:
            if index < len(self.score_samples):
                ...
                solution = {
                    "rating": example.get("gt_score_norm", None),
        ...
        if "image" in example:
            ...
            while not os.path.exists(image_path):
                ...
                image_path = os.path.join(image_root, example["image"])
            image = Image.open(image_path).convert("RGB").resize((300, 224))
        sample["image"] = image
            ...
            if match_answer:
                ...
                if t == 'score':
                    ...
                    if match_score:
                        ...
                        if abs(model_score - true_sol["rating"]) < score_reward_threshold:
                            reward += 0.7
                            match_think = re.search(think_tag_pattern, content, re.DOTALL)
                            if match_think:
                                model_think = match_think.group(1).strip()
                                gt_desc = true_sol["description"]
                                nlp_score = scorer.score(gt_desc, model_think)
                                reward += 0.3 if nlp_score["rougeL"].fmeasure > 0.25 else 0
